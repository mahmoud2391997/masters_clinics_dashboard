...
export const getDevices = async (): Promise<Device[]> => {
  const response = await axios.get<Device[]>(`http://localhost:3000/devices`, getAuthHeaders());
...
export const deleteDevice = async (id: string): Promise<{ message: string }> => {
  const response = await axios.delete<{ message: string }>(`http://localhost:3000/devices/${id}`, getAuthHeaders());
  return response.data;
};
