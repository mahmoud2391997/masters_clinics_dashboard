...
const MedicalDevicesPage: React.FC = () => {
  ...
  const handleOk = async () => {
    try {
      ...
      const formattedSlots = values.working_time_slots.map((slot: any) => {
      ...
      if (editingDevice) {
        ...
        setDevices(devices.map(device => 
          device._id === editingDevice._id ? updatedDevice : device
        ));
                    ...
                    <Space key={key} style={{ display: 'flex', marginBottom: 8 }} align="baseline">
                      ...
                      <Form.Item
                        ...
                        shouldUpdate={(prevValues, currentValues) =>
                          prevValues.working_time_slots?.[name]?.type !== 
                          currentValues.working_time_slots?.[name]?.type
                        }
                      >
                        {({ getFieldValue }) => {
